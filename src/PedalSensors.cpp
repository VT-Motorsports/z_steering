...
bool PedalSensor::checkOpenCircuit() const {
  ...
  printf("Pedal %d: Open Circuit %s\n", inputPin,
         isOpenCircuit ? "detected" : "not detected");
#endif
...
bool PedalSensor::checkShortCircuit() const {
  ...
  printf("Pedal %d: Short Circuit %s\n", inputPin,
         isShortCircuit ? "detected" : "not detected");
#endif
...
bool pedalAgreementFault(PedalSensor p1, PedalSensor p2) {
  ...
  if (abs(p1.pedalPercentage() - p2.pedalPercentage()) >
      0.1) { // maybe increase if p2 is being really screwy
    if (faultTime == 0) {
